...
public class JdbcCityDao implements CityDao {
    ...
    @Override
    // fetch a city from the database
    public City getCity(int cityId) {
        City city = null;
        String sql = "SELECT city_id, city_name, state_abbreviation, population, area " +
                     "FROM city " +
                     "WHERE city_id = ?;"; // 1 - write query
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, cityId); // 2 - send query to database
        if (results.next()) { // 3 - iterate returned data
            city = mapRowToCity(results); // 4 - map sql row to object instance
        }
        return city; // 5 return objects to caller
    }
    ...
    public List<City> getCitiesByState(String stateAbbreviation) {
        ...
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, stateAbbreviation); // 2
        while (results.next()) { // 3
            cities.add(mapRowToCity(results)); // 4
        }
         return cities; // 5
    }
    ...
    public City createCity(City newCity) {
        String sql = "INSERT INTO newCity (city_name, state_abbreviation, population, area) " +
                     "VALUES (?, ?, ?, ?) RETURNING city_id;";
        Integer newId = jdbcTemplate.queryForObject(sql, Integer.class,
                newCity.getCityName(), newCity.getStateAbbreviation(), newCity.getPopulation(), newCity.getArea());
