  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      boardsService
        ...
        .catch((err) => {
          if (err.response) {
            this.errorMsg = `error saving board. response was ${err.response.statusText}`;
          } else if (err.request) {
            this.errorMsg = `error saving board. something went wrong. probably not your fault. well have someone look into it and get back to you.`;
          } else {
            this.errorMsg =
              "sorry. didn't happen. don't bother hitting the save button again. it won't work.";
          }
        });
    },
