  ...
  methods: {
    ...
    deleteBoard() {
      ...
      boardsService
        ...
        .catch((error) => {
          if (error.response) {
            this.errorMsg = `could not remove this board. ${error.response.statusText}`;
          } else if (error.request) {
            this.errorMsg = `could not remove this board. no response from the server`;
          } else {
            this.errorMsg = `not happenin`;
          }
        });
    },
  },
