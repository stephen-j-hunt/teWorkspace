...
public class Exercises {
    ...
    public String animalGroupName(String animalName) {
        ...
        if (animalName != null) {
            ...
            if (animals.containsKey(animalName.toLowerCase())) {
                return animals.get(animalName.toLowerCase(Locale.ROOT));
    ...
    public double isItOnSale(String itemNumber) {
        ...
        result.put("kitchen4001", .20);
        result.put("garage1070", .15);
        result.put("livingroom", .10);
        result.put("kitchen6073", .40);
        result.put("bedroom3434", .60);
        result.put("bath0073", .60);
        if (itemNumber == null) {
            return 0.00;
        }
        if (result.containsKey(itemNumber.toLowerCase())) {
            return result.get(itemNumber.toLowerCase(Locale.ROOT));
        }
        return 0.00;
    ...
    public Map<String, Integer> wordCount(String[] words) {
        ...
        for (String word : words) {
            if (numberWords.containsKey(word)) {
                numberWords.put(word, numberWords.get(word) + 1);
            } else
